...
function Register() {
  ...
  const [chatHistory, setChatHistory] = useState([
    { sender: 'bot', text: 'Xin chào! Bạn cần hỗ trợ gì trong quá trình đăng ký tài khoản không?' }
  ]);
  ...
  const handleRegister = async (e) => {
    ...
    if (result.success) {
      alert(result.message || 'Đăng ký tài khoản thành công! Vui lòng đăng nhập.');
      navigate('/login');
    } else {
  ...
  const handleSendChat = (e) => {
    ...
    setTimeout(() => {
      setChatHistory((prev) => [
        ...prev,
        { sender: 'bot', text: 'Cảm ơn bạn. Yêu cầu hỗ trợ đăng ký của bạn đã được tiếp nhận.' }
      ]);
    }, 1000);
  };
